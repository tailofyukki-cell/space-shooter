...
通常利用では、プロジェクト直下の `PLAY_ASTRAL_BLOOM.vbs` をダブルクリックしてください。ローカルサーバーを非表示で起動し、ブラウザで `http://127.0.0.1:4173/` を開きます。コマンドプロンプトは表示されません。

`PLAY_ASTRAL_BLOOM.bat` は既存の起動導線との互換用です。こちらも非表示ランチャーへ委譲しますが、Windows上では `.vbs` を直接起動する方法を推奨します。
